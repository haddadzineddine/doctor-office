...
export class AppointmentsService {
  ...
  async create(doctorId: number, createAppointmentDto: CreateAppointmentDto) {
    ...
    const appointmentAlreadyExists = await this.canCreateAppointment(doctorId, rest.date, rest.time);

    if (appointmentAlreadyExists) {
      throw new HttpException('Appointment already exists at this date & time', HttpStatus.BAD_REQUEST);
    }
  ...
  private async canCreateAppointment(doctorId: number, date: Date, time: string): Promise<boolean> {
